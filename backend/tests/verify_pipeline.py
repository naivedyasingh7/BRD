...
class TestBRDGenieBackend(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # Backup existing database if any, to avoid corrupting user's data
    ...
    def test_database_versioning_and_auditing(self):
        ...
        # Save Version 2 (same project name)
        brd_id_2 = save_brd(
            project_name=project_name,
            raw_input=raw_input,
            cleaned_input=cleaned_input,
    ...
    def test_graph_compilation(self):
        # Assert that the LangGraph workflow compiled successfully
    ...
    def test_schemas_validation(self):
        # Pydantic validation of clarify request payload
        mock_payload = {
            "state": {
                "raw_input": "Test text",
