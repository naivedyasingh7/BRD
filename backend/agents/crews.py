...
flowchart_agent = Agent(
    role="System Architecture Visualizer",
    goal="Generate valid Mermaid.js graph TD flowchart syntax representing the system architecture or flow based on the BRD.",
    backstory="You are a system architect who translates requirements into clear, visual Mermaid.js flowcharts.",
    llm=shared_groq_llm,
    verbose=True
)
...
def build_flowchart_task(brd_draft: str) -> Task:
    return Task(
        description=f"Based on the following BRD, generate a Mermaid.js flowchart (graph TD) showing the high-level system architecture or user flow. Return ONLY the raw Mermaid syntax starting with `graph TD`, without any markdown code blocks (like ```mermaid) or other text.\n\nBRD:\n{brd_draft}",
        expected_output="Valid Mermaid syntax starting with graph TD.",
