...
def run_crew_with_retry(crew: Crew) -> str:
    for attempt in range(6):
        try:
            return crew.kickoff()
        except Exception as e:
            if "RateLimitError" in str(e) or "429" in str(e) or "rate limit" in str(e).lower():
                logger.warning(f"Rate limit hit! Sleeping for 20 seconds before retry {attempt+1}/6...")
                time.sleep(20)
            else:
                raise e
...
def extract_agent(state: BRDState) -> dict:
    ...
    try:
        res_context = str(run_crew_with_retry(crew_context))
    ...
    try:
        res_requirements = str(run_crew_with_retry(crew_requirements))
    ...
    try:
        res_stakeholders = str(run_crew_with_retry(crew_stakeholders))
...
def clarify_agent(state: BRDState) -> dict:
    ...
    try:
        res_ambiguity = run_crew_with_retry(crew_ambiguity)
    ...
    try:
        res_risk = str(run_crew_with_retry(crew_risk))
...
def brd_agent(state: BRDState) -> dict:
    ...
    if brd_draft and answers:
        ...
        try:
            res_refine = str(run_crew_with_retry(crew_refine))
    ...
    try:
        res_structure = str(run_crew_with_retry(crew_structure))
    ...
    try:
        res_write = str(run_crew_with_retry(crew_write))
    ...
    try:
        res_qa = str(run_crew_with_retry(crew_qa))
    ...
    tracker.start_trace(step_name="Documentation: Flowchart", agent_role="System Architecture Visualizer")
    task_flowchart = build_flowchart_task(res_qa)
    crew_flowchart = Crew(agents=[task_flowchart.agent], tasks=[task_flowchart], process=Process.sequential, verbose=True)
    try:
        res_flowchart = str(run_crew_with_retry(crew_flowchart))
        res_flowchart = res_flowchart.replace("```mermaid", "").replace("```", "").strip()
        tracker.end_trace(output_text=res_flowchart, input_text=res_qa, status="Success")
    except Exception as e:
        tracker.end_trace(output_text=str(e), input_text=res_qa, status="Failed")
        res_flowchart = "graph TD\n    A[Error generating flowchart] --> B[Failed]"
    agent_traces = state.get("agent_traces", []) + tracker.traces
    return {
        ...
        "qa_review": res_qa,
        "final_brd": res_qa,
        "mermaid_flowchart": res_flowchart,
        "agent_traces": agent_traces
    }
