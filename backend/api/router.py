...
from backend.api.schemas import ClarifyRequest, BRDStateResponse, ProjectHistoryResponse, BRDDetailResponse, ChatRequest, ChatResponse
...
ALLOWED_TEXT_EXTENSIONS = {".txt", ".pdf", ".docx", ".md", ".json", ".csv"}
    ...
    try:
        ...
        if not questions:
            ...
            db_id = save_brd(
                ...
                mermaid_flowchart=clean_state.get("mermaid_flowchart", "")
            )
...
async def chat_session(request: ChatRequest):
    logger.info(f"POST /api/chat received for BRD ID {request.brd_id}")
    brd = get_brd_by_id(request.brd_id)
    if not brd:
        raise HTTPException(status_code=404, detail="BRD not found.")
        
    transcript = brd.get("cleaned_input", "")
    if not transcript:
        return ChatResponse(answer="I'm sorry, but I don't have a transcript for this document to answer questions about.")
        
    from backend.services.llm import shared_groq_llm
    from langchain_core.messages import HumanMessage, SystemMessage
    
    sys_msg = SystemMessage(content="You are a helpful assistant analyzing an audio transcript for a user. Use the provided transcript to answer the user's question accurately and concisely. If the answer is not in the transcript, say so.")
    user_msg = HumanMessage(content=f"Transcript:\n{transcript}\n\nQuestion:\n{request.question}")
    
    try:
        response = shared_groq_llm.invoke([sys_msg, user_msg])
